...
describe('Subscription Change', () => {
  ...
  beforeAll(async () => {
    if (!canTest()) {
      console.warn(
        'Skipping Subscription Change tests due to configuration or environment issues.',
      );
      return;
    }

    const moduleRef = await Test.createTestingModule({
      imports: [RecurlyV3Module],
    }).compile();
  ...
  describe('Preview and Create', () => {
    it('should preview a subscription change', async () => {
      const preview = await changeService.previewSubscriptionChange(
        createdSubscription.id as string,
        {
    ...
    it('should create a subscription change', async () => {
      createdChange = (await changeService.createSubscriptionChange(
        createdSubscription.id as string,
        {
          plan_code: testPlan2.code,
          timeframe: 'now',
  ...
  describe('Read and Delete', () => {
    it('should get a pending subscription change', async () => {
      const change = await changeService.getSubscriptionChange(
        createdSubscription.id as string,
      );
    ...
    it('should remove a pending subscription change', async () => {
      await changeService.removeSubscriptionChange(
        createdSubscription.id as string,
      );
      // Verify that the change is gone by trying to get it again
      await expect(
        changeService.getSubscriptionChange(createdSubscription.id as string),
      ).rejects.toThrow();
    });
  });
  ...
  afterAll(async () => {
    ...
    try {
      if (createdSubscription?.id) {
        await subscriptionsService.terminateSubscription(
          createdSubscription.id as string,
          'none',
        );
      }
